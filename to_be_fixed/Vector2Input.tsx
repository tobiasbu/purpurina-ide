...
export default class Vector2Input extends React.Component<
  IVector2InputProps,
  IVector2InputState
> {
  state = {
  ...
  changeAxis(delta: number): void {
    if (this.labelDown) {
      let str =
        this.changeAxisTarget === 'x-axis' ? this.state.x : this.state.y;
  ...
  private mouseUp = () => {
    ...
    if (
      this.changeAxisInitialValue.toString() !== this._xAxisInput.current.value
    ) {
      this._xAxisInput.current.setSelectionRange(
        0,
        this._xAxisInput.current.value.length,
        'forward'
      );
    }
    // this._xAxisLabel.classList.remove('focus')
    ...
    return (
      <div className="flex-container">
        ...
        <div className="flex-horizontal" style={styleVec}>
          <label
            className="text-label number"
            title="x-axis"
            ref={(child) => {
              this._xAxisLabel = child;
            }}
          ...
          <input
            ...
            name="x-axis"
            ref={this._xAxisInput}
            onChange={this.handleChange}
            onBlur={this.onInputBlur}
            onFocus={this.onInputFocus}
            value={this.state.x}
          />
          <label className="text-label number" style={{ marginLeft: '4px' }}>
            Y
          </label>
