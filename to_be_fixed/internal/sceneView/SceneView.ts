...
import {
  computeTransform2D,
  computeBounds2D,
} from '../../../engine/math/transform/compute';
...
export default class SceneView {
  ...
  init() {
    this._emitter.on(
      'zoom',
      (delta: number, zoomPoint: IVector2) => {
    ...
    this._emitter.on(
      ...
      (cursorPosition: IVector2) => {
        ...
        for (let i = 0; i < list.length; i++) {
          let b = new Bounds2D();
          computeBounds2D(b, list[i].transform.matrix, 100, 100, {
            x: 0,
            y: 0,
          });
