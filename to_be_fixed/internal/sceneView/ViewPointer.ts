  ...
  startListeners() {
    ...
    this.target.addEventListener(
      ...
      (event: WheelEvent) => {
        ...
        if (
          Pointer.mode != PointerMode.None &&
          Pointer.mode != PointerMode.ViewZoom
        ) {
          return;
        }
      ...
      (event: MouseEvent) => {
        ...
        if (Pointer.mode != PointerMode.None) {
          ...
          if ((event.buttons & (4 || 0)) > 0) {
            ...
            this.emitter.emit(
              'select',
              this._cursorTransform.transform(Pointer.position)
            );
          }
        }
    ...
    Events.on('sceneView_mousemove', (position: IVector2, delta: IVector2) => {
      ...
      if (
        Pointer.mode != PointerMode.ViewMoving &&
        Pointer.mode != PointerMode.ViewSelectionArea
      ) {
        return;
      }
