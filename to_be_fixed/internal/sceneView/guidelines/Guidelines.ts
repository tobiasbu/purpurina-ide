...
export default class Guidelines {
  ...
  update(view: View) {
    ...
    const scaledSpacing =
      this.baseSpacing * camera.resolution * view.aspectRatio;
    const nextSpacingScaled =
      this.nextSpacing * camera.resolution * view.aspectRatio;
    ...
    let skipStartY = MathUtils.floor(
      (this.mainGrid.parallax.y / scaledSpacing) * 10
    );
    let skipStartX = MathUtils.floor(
      (this.mainGrid.parallax.x / scaledSpacing) * 10
    );
