...
export default class List<T> {
  ...
  push(child: T): T {
    if (this.unique) {
      if (this.indexOf(child) === -1) this.list.push(child);
    } else {
  ...
  pushFront(child: T): T {
    if (this.unique) {
      if (this.indexOf(child) === -1) this.list.unshift(child);
    } else {
  ...
  insert(child: T, index?: number) {
    ...
    if (this.list.length === 0) return this.push(child);

    if (index >= 0 && index <= this.list.length) {
      if (this.unique) {
        if (this.indexOf(child) === -1) this.list.splice(index, 0, child);
      } else {
  ...
  erase(child: T): T {
    ...
    if (idx !== -1) childToErase = this.list.splice(idx, 1);
  ...
  eraseAt(index: number): T {
    ...
    if (child) this.list.splice(index, 1);
  ...
  splice(start: number, count: number, items?: T[]) {
    if (items === undefined) return this.list.splice(start, count);
    else return this.list.splice(start, count, ...items);
  }
  ...
  swap(childA: T, childB: T): List<T> {
    ...
    if (idx0 < 0 || idx1 < 0) {
      throw new Error(
        'DataList.swap: Could not swap childrens. The objects are not in the list.'
      );
    }
  ...
  swapByIndex(indexA: number, indexB: number): List<T> {
    ...
    if (cA === undefined || cB === undefined) {
      throw new Error(
        'DataList.swapByIndex: Could not swap childrens by index. The objects are not in the list.'
      );
    }
