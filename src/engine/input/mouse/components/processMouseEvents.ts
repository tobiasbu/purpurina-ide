import {
  processKeyButtonDown,
  processKeyButtonUp,
} from '../../keyButton/processKeyButtonEvents';
...
function computeMousePos(
  ...
  mouse.position.x = MathUtils.floor(
    mouse.normalizedPosition.x * inputData.canvas.width
  );
  mouse.position.y = MathUtils.floor(
    mouse.normalizedPosition.y * inputData.canvas.height
  );
}
 ...
 */
export function processMouseButtonDown(
  event: MouseEvent,
  button: KeyButtonControl,
  mouse: MouseSystem,
  inputData: SharedInputData
) {
  if (event.buttons) {
  ...
  mouse.clientPosition.set(
    inputData.clientRectTransformX(event.pageX),
    inputData.clientRectTransformY(event.pageY)
  );
 ...
 * @param { InputData } inputData
 */
export function processMouseButtonUp(
  event: MouseEvent,
  button: KeyButtonControl,
  mouse: MouseSystem,
  inputData: SharedInputData
...
export function processMouseWheel(
  event: WheelEvent | MouseWheelEvent,
  mouse: MouseSystem
) {
  mouse.lastEvent = event;
