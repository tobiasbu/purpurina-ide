...
type ImageSmoothingVendorPrefix =
...
function getImageSmoothPrefix(
  context: CanvasRenderingContext2D | WebGLRenderingContext
): ImageSmoothingVendorPrefix {
  const vendors = ['i', 'webkitI', 'msI', 'mozI', 'oI'];
  for (let i = 0; i < vendors.length; i++) {
    const s = vendors[i] + 'mageSmoothingEnabled';

    if (context[s] !== undefined) return s as ImageSmoothingVendorPrefix;
  }
...
export default class CanvasSmoothing {
  ...
  setEnable(flag?: boolean) {
    ...
    if (this.imageSmoothPrefix) this.context[this.imageSmoothPrefix] = flag;
  }
