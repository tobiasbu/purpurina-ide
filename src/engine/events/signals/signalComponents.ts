...
export function addBinding(signal: Signal, binding: SignalBinding): void {
  ...
  do {
    ...
    signal._bindings.at(n) &&
    binding.priority <= signal._bindings.at(n).priority
  );

  signal._bindings.insert(binding, n);
}

export function indexOfListener(
  signal: Signal,
  listener: EventCallback,
  context: object
): number {
  let r = signal._bindings.each((element: SignalBinding, index: number) => {
...
export function validateListener(listener: EventCallback, func: string): void {
  if (typeof listener !== 'function')
    throw new Error(
      'Signal.{fn}: Listener should be a function.'.replace('{fn}', func)
    );
}
